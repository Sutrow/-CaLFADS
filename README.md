...
Увеличение числа факторов с 30 до 50 и размерности контроллера с 4 до 8 **улучшило декодирование** по сравнению с v3. Позиция X из факторов: 0.831 (v4) vs 0.750 (v3). Факторы LFADS побеждают PCA по большинству переменных: X = 0.831 и 0.756, объекты = 0.274 и 0.231. 
...
Увеличение числа факторов и размерности контроллера помогло декодированию (факторы обогнали PCA). Но я не буду увеличивать кол-во факторов до 50, потому что просто теряется смысл работы)

Нужно решить проблему: линейный декодер не может восстановить разнообразие 249 нейронов из 50 факторов с достаточной амплитудой. В следующей версии заменим его на нелинейный.
...
**Медиана корреляции реконструкции = 0.793.** Это значит: для среднего нейрона модель объясняет ~63% дисперсии сигнала ($r^2 = 0.793^2$ ≈ 0.63). Было 0.63 ($r^2$ ≈ 0.40) в v3. Ноль нейронов с отрицательной корреляцией (были в v4).
