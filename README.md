...
**Обнаружена проблема: отрицательные корреляции.** На реконструкции видно, что для части нейронов красная линия (предсказание LFADS) идёт в противоположную сторону от данных. Нейрон 43 показал корреляцию r = −0.345 на сегменте, модель предсказывает противоположную динамику. Такие нейроны портят общую картину.
