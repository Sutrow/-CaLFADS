...
**Обнаружена проблема: модель не следит за динамикой части нейронов (появляются отрицаательные корреляции)** На графиках видно, что для нейронов 38, 43 и 18 красная линия (LFADS) почти горизонтальная прямая, которая едва меняется, в то время как синие данные показывают резкие пики и провалы. Модели не хватает динамики она предсказывает примерно одно и то же значение на всех 100 кадрах.
