...
5. **Правильная сборка непрерывных факторов:** вместо склейки независимых сегментов (что давало 23 disconnected components в Isomap) — усреднение факторов в зонах перекрытия. Каждый кадр покрывается ~5 сегментами, их факторы усредняются → гладкий непрерывный ряд.

6. **Per-neuron noise model:** для каждого нейрона модель учит свой уровень шума σₙ. Нейроны с высоким SNR получают маленький σ (модель «уверена»), шумные нейроны — большой σ (модель «признаёт незнание»). Это позволяет модели фокусироваться на информативных нейронах, не тратя ёмкость на шумные.
