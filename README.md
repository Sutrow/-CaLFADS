...
Увеличили число факторов `fac_dim` с 30 до 50 (чтобы лучше покрыть 249 нейронов) и размерность выхода контроллера `co_dim` с 4 до 8. Обучили 5 моделей с разными значениями `kl_co_max`: 0.0001, 0.001, 0.005, 0.01, 0.05.
...
| kl_co_max | Valid loss | R² скорости | R² позиции X | R² позиции Y |
|-----------|-----------|-------------|-------------|-------------|
...
**Обнаружена проблема: отрицательные корреляции.** При детальном просмотре реконструкции обнаружилось, что для части нейронов красная линия (предсказание LFADS) идёт в противоположную сторону от данных. Нейрон 43 показал корреляцию r = −0.345 — модель систематически предсказывает противоположную динамику. Такие нейроны портят общую картину.

### Вывод

Сила контроллера мало влияет на качество декодирования — основная поведенческая информация уже закодирована в начальных условиях и генераторе. Но нужно решить проблему отрицательных корреляций.

**Графики:** `figures/v4_kl_co_scan.png`, `figures/v4_negative_correlations.png`
